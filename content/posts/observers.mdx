...
<Term id="mutation-observer">MutationObserver</Term>는 DOM 트리의 변경을 감지합니다. 자식 노드가 추가되거나 삭제될 때, 속성이 변경될 때, 텍스트 콘텐츠가 바뀔 때 콜백이 호출됩니다.
...
<Term id="intersection-observer">IntersectionObserver</Term>는 요소가 뷰포트(또는 지정한 루트 요소)와 **교차하는 순간**을 감지합니다. 스크롤 이벤트를 직접 다루는 것보다 훨씬 성능이 좋습니다.
