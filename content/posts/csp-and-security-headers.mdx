...
**frame-src** -- `<iframe>`으로 임베드할 수 있는 출처를 제한합니다. 클릭재킹 방어에도 도움이 됩니다.
...
const securityHeaders = [
  {
    ...
    value: "default-src 'self'; script-src 'self' 'nonce-{nonce}'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://cdn.example.com; connect-src 'self' https://api.example.com",
  },
  {
