...
fn(); // undefined (strict mode) 또는 "" (non-strict, this가 window)
```
...
단, 생성자 함수가 명시적으로 **객체를 반환**하면 그 객체가 대신 사용됩니다. 원시값을 반환하면 무시되고 새 객체가 사용됩니다.

```javascript
function Odd() {
  this.name = "original";
  return { name: "override" }; // 객체 반환 → 이것이 사용됨
}
new Odd(); // { name: "override" }
```
...
React 클래스 컴포넌트에서 이벤트 핸들러의 `this` 바인딩 문제는 대표적인 실전 사례입니다. `constructor`에서 `this.handleClick = this.handleClick.bind(this)`로 바인딩하거나, 클래스 필드의 화살표 함수를 사용하는 두 패턴이 널리 쓰였습니다. React의 함수형 컴포넌트에서는 `this`를 사용하지 않으므로 이 문제 자체가 사라졌습니다.
