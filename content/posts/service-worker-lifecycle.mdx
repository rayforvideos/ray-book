...
**스코프**가 중요합니다. SW는 자신의 스코프 내의 페이지만 제어할 수 있습니다. `/sw.js`가 루트에 있으면 전체 사이트를 제어할 수 있지만, `/app/sw.js`에 있으면 `/app/` 하위 경로만 제어합니다. SW 파일의 위치가 곧 최대 스코프입니다.

```javascript
// /app/sw.js는 /app/ 하위만 제어 가능
navigator.serviceWorker.register("/app/sw.js");
// scope를 명시해도 파일 위치보다 넓게 설정할 수 없음
...
브라우저는 SW 파일에 대해 **HTTP 캐시를 최대 24시간만** 존중합니다. 24시간이 지나면 `Cache-Control` 헤더와 관계없이 네트워크에서 새로 가져옵니다. 이것은 잘못된 `Cache-Control: max-age=31536000` 설정으로 SW가 영원히 갇히는 상황을 방지하기 위한 안전장치입니다.
