...
Map은 키의 자유로움 덕분에 라이브러리 내부에서 자주 사용됩니다. Node.js의 `EventEmitter`는 이벤트 이름을 키로 리스너 목록을 관리할 때 Map과 유사한 구조를 사용하며, React의 Fiber 재조정 (reconciliation) 과정에서도 `key` prop을 기반으로 자식 요소를 매핑할 때 Map 자료구조를 활용합니다. Set은 고유값 관리에 자주 쓰이며, 위에서 본 배열 중복 제거 외에도 CSS-in-JS 라이브러리에서 이미 주입된 스타일을 추적하는 데 활용됩니다.
