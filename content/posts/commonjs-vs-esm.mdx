...
현대 빌드 도구도 이 흐름을 반영합니다. **Vite** 는 개발 환경에서 네이티브 ESM을 직접 브라우저에 제공하고, npm 패키지 중 CJS로 배포된 의존성은 사전 번들링 (dependency pre-bundling) 단계에서 ESM으로 변환합니다. **Next.js** 역시 ESM을 기본으로 채택하고, `next.config.mjs` 형태의 ESM 설정 파일을 지원합니다.
