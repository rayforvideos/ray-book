...
- **Young Generation** — 새로 생성된 객체가 들어가는 작은 영역 (보통 1~8MB)
...
여러 번의 Minor GC에서 살아남은 객체는 Old Generation으로 승격됩니다. 승격 기준:

- Minor GC에서 **두 번 이상** 살아남았거나
- To-space의 사용률이 **25%를 초과** 할 때
...
function createHandler() {
  ...
  return function handler() {
    // largeData를 쓰지 않지만, 클로저가 참조를 유지
    console.log("clicked");
  };
}
// handler가 살아있는 한 largeData도 GC되지 않음
```
