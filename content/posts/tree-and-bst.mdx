...
```text
        A           <- 루트 (Root): 최상위 노드
       / \
      B   C         <- B, C는 A의 자식 (Children)
     / \   \           A는 B, C의 부모 (Parent)
    D   E   F       <- D, E, F는 리프 (Leaf): 자식이 없는 노드
```
...
`document.querySelector`, `parentNode`, `children` 같은 DOM API는 모두 이 트리를 순회하는 연산입니다. 이전 글에서 다룬 BFS/DFS가 여기에 직접 적용됩니다.
