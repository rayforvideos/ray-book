...
Cookie는 가장 오래된 저장소입니다. 1994년 Netscape에서 장바구니 구현을 위해 도입했습니다. 핵심 특징은 HTTP 요청에 자동으로 포함된다는 점입니다. 이것이 서버와 상태를 공유하는 유일한 클라이언트 저장소인 이유이고, 동시에 [CSRF 공격](/posts/csrf)의 원인이 되는 이유이기도 합니다.
