              ...
              {lines.map((line, i) => {
                ...
                return (
                  <div key={i} className={`flex transition-colors duration-150 ${isActive ? "bg-accent/10" : ""}`}>
                    <span className={`select-none w-7 shrink-0 text-right pr-2 ${isActive ? "text-accent" : "text-muted/30"}`}>{i + 1}</span>
                    <span className={`flex-1 pr-2 py-px ${isActive ? "text-text" : "text-muted/40"}`}>{line || "\u00A0"}</span>
