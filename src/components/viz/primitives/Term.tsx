...
const terms: Record<string, TermDef> = {
  ...
  "big-o": {
    title: "Big-O 표기법",
    emoji: "📈",
    description: "알고리즘의 시간/공간 복잡도를 입력 크기(n)에 대한 상한으로 표현하는 표기법. O(1), O(n), O(n²) 등으로 성능을 비교합니다.",
    analogy: "최악의 교통 상황에서 걸리는 시간과 비슷합니다 — 평균이 아니라 최악을 기준으로 예상합니다.",
  },
  "linked-list": {
    title: "링크드 리스트",
    emoji: "🔗",
    description: "각 노드가 데이터와 다음 노드를 가리키는 포인터로 구성된 자료구조. 삽입/삭제는 O(1)이지만 인덱스 접근은 O(n)입니다.",
    analogy: "보물찾기 게임과 비슷합니다 — 각 단서가 다음 단서의 위치를 알려주므로, 처음부터 순서대로 따라가야 합니다.",
  },
  "bst": {
    title: "이진 탐색 트리",
    emoji: "🌲",
    description: "각 노드의 왼쪽 자식은 더 작은 값, 오른쪽 자식은 더 큰 값을 가지는 이진 트리. 균형 상태에서 탐색/삽입/삭제가 O(log n)입니다.",
    analogy: "사전에서 단어를 찾는 것과 비슷합니다 — 중간을 펼쳐서 앞/뒤를 판단하고 절반씩 좁혀갑니다.",
  },
  "bfs": {
    title: "너비 우선 탐색",
    emoji: "🌊",
    description: "BFS (Breadth-First Search). 그래프에서 시작 노드의 이웃을 먼저 방문하고, 그 다음 깊이의 노드를 방문하는 탐색 알고리즘. 큐를 사용합니다.",
    analogy: "연못에 돌을 던졌을 때 퍼지는 물결과 비슷합니다 — 중심에서 가까운 곳부터 동심원으로 퍼져나갑니다.",
  },
  "dfs": {
    title: "깊이 우선 탐색",
    emoji: "🕳️",
